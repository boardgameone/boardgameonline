/**
 * 3D icosahedron renderer for the CubeTac "Icosahedron" variant.
 *
 * Draws all 20 triangular faces with their 9 cells (6 perimeter up-triangles
 ...
 * `IcosaRotateControls` panel, not by clicking the face itself (no fixed-point
...
/**
 * 20 distinct pastel face colors. The first 12 mirror MegaminxScene's palette
 * for visual continuity; the trailing 8 are additional pastels with good
 * contrast against neighbors so adjacent faces never collide.
 */
const FACE_COLORS: string[] = [
    ...
    '#fef3c7', // yellow-100
    '#e9d5ff', // purple-200
    // 8 additional pastels for icosahedron's 20 faces:
    '#fda4af', // salmon (rose-300)
    '#c4b5fd', // lavender (violet-300)
    '#a7f3d0', // mint (emerald-200)
    '#99f6e4', // teal (teal-200)
    '#fef9c3', // cream (yellow-100 lighter)
    '#c7d2fe', // periwinkle (indigo-200)
    '#fecdd3', // peach-rose (rose-200)
    '#fb923c', // coral (orange-400 — pop accent)
];

const FALLBACK_GLYPH_COLOR = '#94a3b8';
const ANIMATION_MS = 240;

export type IcosaCellClickHandler = (face: number, slot: number) => void;
export type IcosaRotateHandler = (face: number, direction: Direction) => void;

export interface IcosahedronSceneHandle {
...
interface IcosahedronSceneProps {
    marks: Marks;
    ...
    onCellClick?: IcosaCellClickHandler;
 ...
 * ring cells on neighbors. Matches `IcosahedronCube::buildFacePermutation` in PHP.
 */
...
const IcosahedronScene = forwardRef<IcosahedronSceneHandle, IcosahedronSceneProps>(function IcosahedronScene(
    ...
    return (
        <Canvas
            camera={{ position: [3.0, 2.4, 3.2], fov: 45 }}
...
// ---------------------------------------------------------------------------
// Arrow-key camera orbit — held arrow keys spin the icosahedron in space.
// ---------------------------------------------------------------------------
...
interface IcosahedronGroupProps extends Omit<IcosahedronSceneProps, 'marks'> {
    marks: Marks;
...
const IcosahedronGroup = memo(function IcosahedronGroup({
    marks,
    playerColors,
    designs,
    winningIndices,
...
function FaceMesh({
    ...
    // inset gaps so the puzzle surface looks like a real Icosahedron with
...
/** Public accessor for the rotate-controls panel — each face button shows a
 *  matching color swatch so a player can pair "the salmon button rotates the
 *  salmon face". */
export function getFaceColors(): string[] {
    return FACE_COLORS;
}
