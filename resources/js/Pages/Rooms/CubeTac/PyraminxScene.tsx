/**
 * 3D tetrahedron renderer for the CubeTac "Pyraminx" variant.
 *
 * Draws all 4 triangular faces with their 9 cells (6 perimeter up-triangles
...
/** A pleasantly distinct color per face — matches the photo's yellow/red/gray/green. */
...
export type PyraCellClickHandler = (face: number, slot: number) => void;
export type PyraRotateHandler = (face: number, direction: Direction) => void;

export interface PyraminxSceneHandle {
...
interface PyraminxSceneProps {
    ...
    onCellClick?: PyraCellClickHandler;
 ...
 * ring cells on neighbors. Matches `PyraminxCube::buildFacePermutation` in PHP.
 */
...
const PyraminxScene = forwardRef<PyraminxSceneHandle, PyraminxSceneProps>(function PyraminxScene(
...
export default PyraminxScene;

// ---------------------------------------------------------------------------
// Arrow-key camera orbit — held arrow keys spin the tetrahedron in space.
// ---------------------------------------------------------------------------
...
interface TetrahedronGroupProps extends Omit<PyraminxSceneProps, 'marks'> {
    marks: Marks;
...
const TetrahedronGroup = memo(function TetrahedronGroup({
    marks,
    playerColors,
    designs,
    winningIndices,
...
function FaceMesh({
    ...
    // inset gaps so the puzzle surface looks like a real Pyraminx with
