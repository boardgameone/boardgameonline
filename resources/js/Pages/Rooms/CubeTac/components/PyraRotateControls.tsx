/**
 * Rotate-face control panel for the Pyraminx variant.
 *
 * The tetrahedron has 4 faces; each gets a CW/CCW button pair, matching
...
import { Direction } from '@/lib/pyraminx';

interface PyraRotateControlsProps {
...
interface FaceConfig {
    name: string;
    /** Index 0..3 matching `pyraminx.ts` faces. */
    index: number;
    tint: {
        bg: string;
        hoverBg: string;
...
export default function PyraRotateControls({ onRotate, disabled }: PyraRotateControlsProps) {
    // Keyboard shortcut: 1..4 trigger CW rotation of face 0..3; Shift+1..4 CCW.
    useEffect(() => {
        if (disabled) {
            return;
        }
