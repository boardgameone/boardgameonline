...
export default function TwentyEightTutorial({ show, onClose }: TwentyEightTutorialProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
            <div
                className="bg-white rounded-2xl p-6 shadow-2xl max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto dark:bg-gray-800 sepia:bg-sepia-surface"
                onClick={e => e.stopPropagation()}
            >
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xl font-black text-gray-900 dark:text-gray-100 sepia:text-sepia-text">
                ...
                <div className="space-y-4 text-sm text-gray-700 dark:text-gray-300 sepia:text-sepia-text">
                    ...
                    <Section title="Cards & Points">
                        ...
                        <div className="mt-2 grid grid-cols-4 gap-1 text-center text-xs">
                            {[
                                ...
                                <div key={c.rank} className="bg-gray-50 rounded-sm p-1.5 dark:bg-gray-900/40 sepia:bg-sepia-bg/40">
                                    <div className="font-bold text-gray-900 dark:text-gray-100 sepia:text-sepia-text">{c.rank}</div>
                                    <div className="text-amber-600 font-bold dark:text-amber-400 sepia:text-sepia-accent">{c.pts}pt{c.pts !== 1 ? 's' : ''}</div>
                                </div>
                            ))}
                        </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <div>
            <h4 className="font-bold text-gray-900 mb-1 dark:text-gray-100 sepia:text-sepia-text">{title}</h4>
            {children}
        </div>
    );
}
