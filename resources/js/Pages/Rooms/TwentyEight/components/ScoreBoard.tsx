        ...
        return (
            <div className="flex items-center gap-3 text-xs">
                <div className="flex items-center gap-1">
                    ...
                    <span className="font-bold text-blue-700">{points.team_a}pts</span>
                    <span className="text-gray-400">({gameScores.team_a})</span>
                </div>
                <div className="text-gray-300">|</div>
                <div className="flex items-center gap-1">
                    <span className="inline-block w-2 h-2 rounded-full bg-rose-500" />
                    <span className="font-bold text-rose-700">{points.team_b}pts</span>
                    <span className="text-gray-400">({gameScores.team_b})</span>
                </div>
                {bidTeam && (
                    <>
                        <div className="text-gray-300">|</div>
                        <span className="text-gray-500">Bid: {bidValue}</span>
                    </>
                )}
            </div>
        );
...
function TeamScore({
    ...
    const bgClass = color === 'blue' ? 'bg-blue-50' : 'bg-rose-50';
    const textClass = color === 'blue' ? 'text-blue-700' : 'text-rose-700';
    const dotClass = color === 'blue' ? 'bg-blue-500' : 'bg-rose-500';
    const badgeClass = color === 'blue' ? 'bg-blue-100 text-blue-700' : 'bg-rose-100 text-rose-700';
    ...
    return (
        <div className={`p-3 ${bgClass}`}>
            ...
            <div className="space-y-1">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-500">Tricks</span>
                    <span className="text-sm font-bold text-gray-700">{tricks}</span>
                </div>
                <div className="flex justify-between items-center border-t border-gray-200 pt-1 mt-1">
                    ...
                    <span className={`text-sm font-black ${gameScore >= 0 ? 'text-green-600' : 'text-red-600'}`}>
