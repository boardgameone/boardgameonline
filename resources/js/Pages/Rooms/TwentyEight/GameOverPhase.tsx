    ...
    return (
        <div className="max-w-lg mx-auto space-y-6 py-4">
            ...
            <div className={`rounded-2xl p-8 text-center shadow-xl ${
                didWin
                    ? 'bg-gradient-to-br from-amber-100 via-yellow-100 to-amber-200 border-2 border-amber-400'
            ...
            {/* Winning team */}
            <div className="rounded-xl bg-gradient-to-br from-amber-50 to-yellow-50 shadow-md border border-amber-200 p-5">
                <h4 className="text-sm font-bold text-amber-700 mb-3 text-center">Winners</h4>
                <div className="flex justify-center gap-4">
                    {winningPlayers.map(player => (
                        <div key={player.id} className="text-center">
                            <div
                                className="w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg mx-auto mb-1 ring-2 ring-amber-400 ring-offset-2"
                                style={{ backgroundColor: player.avatar_color }}
                            >
                                {player.nickname.charAt(0).toUpperCase()}
                            </div>
                            <div className="text-sm font-bold text-gray-800">{player.nickname}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {gameState.round_history.length > 0 && (
                <div className="rounded-xl bg-white shadow-md border border-gray-200 p-4">
                    <h4 className="text-sm font-bold text-gray-600 mb-3">Round History</h4>
                    <div className="space-y-2">
                        {gameState.round_history.map((round, idx) => (
                            <div key={idx} className="flex items-center justify-between text-xs bg-gray-50 rounded-lg p-2">
                                <span className="font-medium text-gray-600">R{round.round_number}</span>
                                <span className="text-gray-500">
                                    Bid: {round.bid_value} ({round.bid_team === 'team_a' ? 'A' : 'B'})
                                </span>
                                <div className="flex gap-2">
                                    <span className="text-blue-600 font-bold">A: {round.points.team_a}</span>
                                    <span className="text-rose-600 font-bold">B: {round.points.team_b}</span>
                                </div>
                            </div>
                        ))}
                    </div>
