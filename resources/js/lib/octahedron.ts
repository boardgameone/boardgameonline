/**
 * TypeScript twin of app/Services/OctahedronCube.php.
 *
 * Same vertex/face layout, same adjacency, same permutations. Used by the
 * OctahedronScene component to render the 8-face octahedron, animate face
...
export type Marks = Mark[]; // length 72
...
/** Fresh 72-element mark array (all null). */
...
/** All currently-winning perimeter triples across all 8 faces. */
...
/** Inradius of the octahedron with our vertex coords (1/√3 ≈ 0.577). */
export function inradius(): number {
    return 1 / Math.sqrt(3);
}

/**
 * Raw (un-normalized) face-center direction vectors — the 8 octant vectors
 * `(±1, ±1, ±1)`. Stable enumeration: x outermost, then y, then z (each
 * taking values -1, +1).
 */
...
/** The 8 face center positions (at the inradius along each face normal). */
...
/** Unit outward normals for the 8 faces. */
...
export function firstRingCells(face: number): number[] {
    ...
    for (let i = 0; i < 3; i++) {
        const a = adj[face][i];
        const k = sk[face][i];
        // Per-edge layout (mirror of OctahedronCube::buildFacePermutation):
...
/** lines[]: 48 triples of cell indices forming winning perimeter lines. */
...
/** Permutation tables for all 16 moves, keyed by `${face}:${direction}`. */
...
/**
 * Geometry parameters for the Octahedron face tiling.
 *
...
function cellBarycentric(slot: number): [Vec3, Vec3, Vec3] {
    ...
    switch (slot) {
        ...
        default:
            throw new Error(`Invalid octahedron slot: ${slot}`);
    }
}

/**
 * Compute the polygon vertices (in 3D, in the face plane) for a single
 * Octahedron cell. Returns 3 vertices (each cell is a triangle), already
 * inset by `PIECE_INSET`. CCW order viewed from outside the octahedron.
 */
