/**
 * TypeScript twin of app/Services/IcosahedronCube.php.
 *
 * Same vertex/face layout, same adjacency, same permutations. Used by the
 * IcosahedronScene component to render the 20-face icosahedron, animate face
...
/** Fresh 180-element mark array (all null). */
...
/** All currently-winning perimeter triples across all 20 faces. */
 ...
 * coordinates: (0, ±1, ±φ), (±1, ±φ, 0), (±φ, 0, ±1). Enumeration matches
 * the megaminx face-center direction order (icosa+dodec are duals).
 */
...
/**
 * Inradius of the icosahedron with our vertex coords.
 * (1+φ) / sqrt(3) = φ² / sqrt(3) ≈ 1.512.
 */
export function inradius(): number {
    return (1 + (1 + Math.sqrt(5)) / 2) / Math.sqrt(3);
}

/**
 * Raw (un-normalized) face-center direction vectors. The 20 dodecahedron
...
function faceCenterDirections(): Vec3[] {
    ...
    // (0, ±φ, ±1/φ)
    for (const y of [-phi, phi]) {
        for (const z of [-inv, inv]) {
            dirs.push([0, y, z]);
        }
    }
    // (±1/φ, 0, ±φ)
    for (const x of [-inv, inv]) {
        for (const z of [-phi, phi]) {
            dirs.push([x, 0, z]);
        }
    }
...
/** The 20 face center positions (at the inradius along each face normal). */
...
/** Unit outward normals for the 20 faces. */
...
export function firstRingCells(face: number): number[] {
    ...
    for (let i = 0; i < 3; i++) {
        const a = adj[face][i];
        const k = sk[face][i];
        // Per-edge layout (mirror of IcosahedronCube::buildFacePermutation):
...
/** Permutation tables for all 40 moves, keyed by `${face}:${direction}`. */
...
/**
 * Geometry parameters for the Icosahedron face tiling.
 *
...
function cellBarycentric(slot: number): [Vec3, Vec3, Vec3] {
    ...
    switch (slot) {
        ...
        default:
            throw new Error(`Invalid icosahedron slot: ${slot}`);
    }
}

/**
 * Compute the polygon vertices (in 3D, in the face plane) for a single
 * Icosahedron cell. Returns 3 vertices (each cell is a triangle), already
 * inset by `PIECE_INSET`. CCW order viewed from outside the icosahedron.
 */
