...
/**
 * Pure value-object for the CubeTac "Icosahedron" variant — a 20-face
 * triangular polyhedron (20 triangular faces × 9 cells per face = 180 cells
 * total). Each face is subdivided identically to the Pyraminx face, so the
 * algorithm mirrors `PyraminxCube`; only the geometry (12 vertices, 20 face
 * centers, inradius) differs.
 *
 * Sticker indexing
 * ----------------
 * Flat 180-element array `[int|null, ...]` indexed by `index = face * 9 + slot`.
 * Per face (slot 0–8):
 ...
 * Rotations
 * ---------
 * 40 moves total: 20 faces × {cw, ccw}. A CW turn of face F is defined as the
 * rotation that maps F.v[i] → F.v[(i-1) % 3] (the rotation that visually
 * appears clockwise to a viewer outside the icosahedron). Each turn permutes
 ...
 * on them). Validated by `IcosahedronCubeTest`.
 *
 * Winning lines
 * -------------
 ...
 * edge 2 → back to corner v[0]. 6 perimeter triples per face × 20 faces =
 * 120 lines total. No cross-face wraparound; no diagonals.
 */
final class IcosahedronCube
{
    public const FACES = 20;

    public const CELLS_PER_FACE = 9;

    public const PLAYABLE_PER_FACE = 6;

    public const TOTAL_CELLS = 180;
    ...
    /** @var list<list<int>>|null adjacent face per (face, edge_index) — 20×3 */
    ...
    /**
     * Return a fresh 180-element mark array (all nulls).
     *
    ...
    /**
     * Return all currently-winning lines across all 20 faces.
     *
     * @param  list<int|null>  $marks
    ...
    /**
     * Whether all *playable* cells (the 120 up-triangles, 6 per face) are
     * marked. The 60 dead down-triangles are skipped — they're never marked.
     *
     * @param  list<int|null>  $marks
     */
    ...
    /**
     * The 40 move permutations (memoized).
     *
    ...
    /**
     * 20 unit face-normal directions for animation/rendering. Each entry is
    ...
    public static function moveParams(): array
    {
        ...
        foreach ($normals as $f => $axis) {
            ...
            // counter-clockwise as seen from outside the icosahedron. So the
    ...
    /**
     * Build all 40 permutation tables.
     *
     * @return array<string, list<int>>
     */
    ...
    private static function computeFaceData(): void
    {
        ...
        // (For a regular icosahedron, exactly 3 of the 12 vertices lie on each face.)
        $faceVertices = [];
        for ($f = 0; $f < self::FACES; $f++) {
    ...
    /**
     * The 12 icosahedron vertices, hardcoded with the standard golden-ratio
     * coordinates: (0, ±1, ±φ), (±1, ±φ, 0), (±φ, 0, ±1). Enumeration order
     * matches `MegaminxCube::faceCenterDirections()` (icosahedron and
     * dodecahedron are duals — icosa vertices = dodec face centers).
     *
     * @return list<array{0:float, 1:float, 2:float}>
     */
    ...
    /**
     * The 20 face centers (at the icosahedron's inradius along each face normal).
     *
     * @return list<array{0:float, 1:float, 2:float}>
     */
    ...
    /**
     * Raw face-center direction vectors (not normalized). The 20 dodecahedron
     * vertex directions: (±1,±1,±1) ∪ (0,±φ,±1/φ) ∪ (±1/φ,0,±φ) ∪ (±φ,±1/φ,0).
     * Order is fixed and referenced by face index 0..19 throughout the codebase.
     * Enumeration matches `MegaminxCube::vertices()` (icosa+dodec are duals).
     *
     * @return list<array{0:float, 1:float, 2:float}>
     */
    ...
    /**
     * Inradius of the icosahedron with our vertex coordinates.
     * Vertex magnitude = sqrt(2 + φ) ≈ 1.902; face-center inradius =
     * (1 + φ) / sqrt(3) = φ² / sqrt(3) ≈ 1.512.
     */
    private static function inradius(): float
    {
        return (1.0 + (1.0 + sqrt(5.0)) / 2.0) / sqrt(3.0);
    }
