...
class CubeTacIcosahedronTest extends TestCase
{
    use RefreshDatabase;
     ...
     */
    private function makeRoom(string $variant = 'icosahedron'): array
    {
    ...
    public function test_host_can_pick_icosahedron_variant(): void
    {
        $game = Game::factory()->cubeTac()->create();
        $host = User::factory()->create();
        ...
        $this->actingAs($host)
            ->post(route('rooms.pickVariant', [$game->slug, $room->room_code]), ['variant' => 'icosahedron'])
            ->assertRedirect();

        $this->assertSame('icosahedron', $room->fresh()->variant);
    }

    public function test_starting_an_icosahedron_game_creates_a_180_cell_marks_array(): void
    {
        $data = $this->makeRoom('icosahedron');
        ...
        $this->assertCount(180, $room->settings['marks']);
        $this->assertSame(60, $room->settings['move_limit']); // 30 * 2 players
        $this->assertNull($room->settings['marks'][0]);
    }

    public function test_icosa_mark_rejects_dead_down_triangle_slots(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
        ...
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.icosaMark', [$data['game']->slug, $data['room']->room_code]), [
                'face' => 0,
                'slot' => 6,
            ])
    ...
    public function test_icosa_mark_rejects_other_players_turn(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
    ...
    public function test_icosa_mark_then_end_turn_advances_slot(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
        ...
        $this->assertSame(0, $room->settings['marks'][IcosahedronCube::indexOf(0, 1)]);
    ...
    public function test_icosa_rotate_auto_advances_turn_without_confirm(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
        ...
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.icosaRotate', [$data['game']->slug, $data['room']->room_code]), [
                'face' => 0,
                'direction' => 'cw',
            ])
            ->assertRedirect();
    ...
    public function test_icosa_rotate_rejects_invalid_direction(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
    ...
    public function test_winning_perimeter_triple_ends_the_game(): void
    {
        ...
        $settings['marks'][IcosahedronCube::indexOf(0, 0)] = 0; // host
        $settings['marks'][IcosahedronCube::indexOf(0, 1)] = 0; // host
    ...
    public function test_rematch_keeps_icosahedron_variant(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
        ...
        $room = $data['room']->fresh();
        $settings = $room->settings;
        $settings['marks'][IcosahedronCube::indexOf(0, 0)] = 0;
        $settings['marks'][IcosahedronCube::indexOf(0, 1)] = 0;
        $room->update(['settings' => $settings]);
        ...
        $room->refresh();
        $this->assertSame('playing', $room->status);
        $this->assertSame('icosahedron', $room->variant);
        $this->assertCount(180, $room->settings['marks']);
    }

    public function test_cube_route_rejects_icosahedron_room(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
    ...
    public function test_megaminx_route_rejects_icosahedron_room(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
    ...
    public function test_pyraminx_route_rejects_icosahedron_room(): void
    {
        $data = $this->makeRoom('icosahedron');
        $this->actingAs($data['host'])
            ->post(route('rooms.cubetac.start', [$data['game']->slug, $data['room']->room_code]));
    ...
    public function test_icosahedron_route_rejects_cube_room(): void
    {
